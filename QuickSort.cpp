...
void QuickSort::troca(Rating* &r1, Rating* &r2)
{
...
int QuickSort::particiona(Rating* vet[], int inicio, int fim, int pos)
{
	int pospiv;
	/*
...
void QuickSort::quicksort(Rating* vet[], int ini, int fim, char tipo)
{
	clock_t relogio;
	relogio = clock();
...
int QuickSort::mediana(Rating* vet[], int numVal, int inicio, int fim) {
	...
	if (numVal == 3) { //para k=3
		...
		for (int i = 0; i < 3; i++) {
			...
			vetor[i] = new Rating(posRand, vet[posRand]->getUserID(), "", ""); //Atribui a MovieID a posicao em si, e em UserID o valor do UserID contido nesta posicao
		}
		InsertionSort ordena;
		ordena.insertionsort(vetor, 0, 3);//utiliza o insertionSort para ordenar o UserID do vetor de 3 posicoes
		posMediana = vet[1]->getMovieID();//pega o valor central do vetor (mediana) e atribui o movieID, que é a posicao original do vetor de tweets
		return posMediana;
	}
	...
	if (numVal == 5) {//para k=5
		...
		InsertionSort ordena;
		ordena.insertionsort(vetor, 0, 3);//utiliza o insertionSort para ordenar o UserID do vetor de 3 posicoes
		posMediana = vet[2]->getMovieID();//pega o valor central do vetor (mediana) e atribui o MovieID, que é a posicao original do vetor de tweets
		return posMediana;
	}
